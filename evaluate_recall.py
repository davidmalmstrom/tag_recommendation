...
def get_preds(model, val_x, K):
    def top_cands(user_row, u_num):
        ...
        not_seen_item_indices = np.where(np.squeeze(user_row.toarray()) == 0)[0]
        users = np.full(len(not_seen_item_indices), u_num, dtype='int32')
        predictions = model.predict([users, not_seen_item_indices],
                                    batch_size=100, verbose=0)
        
        map_item_score = {}
        for i in range(len(not_seen_item_indices)):
            item_index = not_seen_item_indices[i]
